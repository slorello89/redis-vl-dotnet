...
* .NET 9 SDK
* Redis 8 or another RediSearch-capable Redis deployment
...
| RediSearch + RedisJSON. Supports `REDIS_VL_REDIS_URL` or `REDIS_VL_REDIS_CLUSTER_NODES`.
...
| Local/offline ONNX vectorization
| xref:../extensions/onnx-vectorizer.adoc[ONNX Vectorizer]
| `/examples/OnnxVectorizerExample`
| Local `model.onnx` and `tokenizer.json` assets exposed through `ONNX_VECTORIZER_MODEL_PATH` and `ONNX_VECTORIZER_TOKENIZER_PATH`. No Redis or provider credentials required.
