...
* `/examples/ExtensionsAiVectorizerExample` uses an in-memory sample `IEmbeddingGenerator<string, Embedding<float>>`
...
The remote-provider examples read credentials from environment variables and fail fast with an explicit message when the required provider credential is missing. The ONNX example follows the same fail-fast pattern for local asset paths. The Microsoft.Extensions.AI adapter example is credential-free because it uses an in-memory sample generator.
