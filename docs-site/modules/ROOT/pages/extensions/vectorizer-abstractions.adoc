...
`RedisVlDotNet.Vectorizers.Abstractions` defines the provider-neutral contracts used by caches and workflow APIs.
...
* xref:extensions/openai-vectorizer.adoc[OpenAI vectorizer] is the batch-capable reference implementation under `src/RedisVlDotNet.Vectorizers.OpenAI`.
* xref:extensions/huggingface-vectorizer.adoc[Hugging Face vectorizer] is the HTTP-based reference implementation under `src/RedisVlDotNet.Vectorizers.HuggingFace`.
* `/examples/OpenAiVectorizerExample` and `/examples/HuggingFaceVectorizerExample` show environment-based provider setup.
* `tests/RedisVlDotNet.Tests/Vectorizers/TextVectorizerExtensionsTests.cs` captures the batch fallback contract.
