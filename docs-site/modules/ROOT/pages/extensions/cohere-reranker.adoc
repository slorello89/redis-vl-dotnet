...
`RedisVlDotNet.Rerankers.Cohere` provides `CohereTextReranker`, an `ITextReranker` implementation backed by Cohere's rerank API.
...
* `tests/RedisVlDotNet.Tests/Rerankers/CohereTextRerankerTests.cs` covers request construction and response validation
* `tests/RedisVlDotNet.Tests/Rerankers/CohereTextRerankerSmokeTests.cs` is the live-provider smoke test gate
