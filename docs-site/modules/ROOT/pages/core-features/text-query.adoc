...
If a required projected field is missing or cannot be converted into the target .NET property type, the library throws `SearchResultMappingException`. The mapping contract is covered in `tests/RedisVlDotNet.Tests/Queries/SearchResultMappingTests.cs`.
