...
Vector fields must include an `attrs` block with algorithm, data type, dimensions, and distance metric. HNSW and SVS-VAMANA fields can set `epsilon` (the range-query approximation factor); SVS-VAMANA fields can also set `compression`, `construction_window_size`, `graph_max_degree`, `search_window_size`, `training_threshold`, and `reduce` in `attrs`.
...
* invalid combinations such as HNSW-only settings on a FLAT vector field, or SVS-VAMANA-only settings on a non-SVS-VAMANA field
