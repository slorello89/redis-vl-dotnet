...
Vector fields must include an `attrs` block with algorithm, data type, dimensions, and distance metric. SVS-VAMANA fields can also set `compression`, `construction_window_size`, `graph_max_degree`, `search_window_size`, `epsilon`, `training_threshold`, and `reduce` in `attrs`.
