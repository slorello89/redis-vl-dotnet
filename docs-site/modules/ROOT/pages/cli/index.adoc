...
If you want a reusable local executable, publish the CLI into a local folder and invoke the generated `RedisVlDotNet.Cli.dll` with `dotnet`:

[source,bash]
----
dotnet publish src/RedisVlDotNet.Cli/RedisVlDotNet.Cli.csproj -c Release -o .artifacts/redisvl-cli
dotnet .artifacts/redisvl-cli/RedisVlDotNet.Cli.dll --help
----
...
[source,bash]
----
dotnet run --project src/RedisVlDotNet.Cli -- index --help
dotnet run --project src/RedisVlDotNet.Cli -- schema --help
----
...
dotnet run --project src/RedisVlDotNet.Cli -- index list
----
...
[source,bash]
----
dotnet run --project src/RedisVlDotNet.Cli -- schema validate \
...
[source,bash]
----
dotnet run --project src/RedisVlDotNet.Cli -- schema show \
  --file examples/JsonStorageExample/schema.yaml
----
...
[source,bash]
----
dotnet run --project src/RedisVlDotNet.Cli -- index info \
...
[source,bash]
----
dotnet run --project src/RedisVlDotNet.Cli -- index clear \
  --redis localhost:6379 \
  --name movies-cli-idx \
  --batch-size 500
----
...
[source,bash]
----
dotnet run --project src/RedisVlDotNet.Cli -- index delete \
  --redis localhost:6379 \
  --name movies-cli-idx
----
